Feature: End-to-End Resolution Cycle
  ...
  #   Phase 1 — Bounded intake (POST /resolve → identifier)
  #   Phase 2 — Authoritative assessment (ERE outcome → Decision Store)
  #   Phase 3 — Convergence (GET /lookup, POST /refreshBulk → observe changes)
  #
  ...
  # ---------------------------------------------------------------------------
  # Main Success — canonical resolution observed via lookup and refreshBulk
  # ---------------------------------------------------------------------------

  Scenario: Submit a mention, receive canonical identifier, verify via lookup and refreshBulk
    ...
    When the originator calls refreshBulk for source "SYSTEM_A"
  ...
  Scenario: Submit a mention, receive provisional identifier, ERE responds late, refreshBulk shows updated cluster
    ...
    When the originator calls refreshBulk for source "SYSTEM_B"
  ...
  # ---------------------------------------------------------------------------
  # Multiple mentions — refreshBulk returns all changes, then empty
  # ---------------------------------------------------------------------------

  Scenario: Submit multiple mentions for the same source and observe all via refreshBulk
    ...
    When the originator calls refreshBulk for source "SYSTEM_E"
    ...
    When the originator calls refreshBulk for source "SYSTEM_E" again
