Feature: Cluster Assignment Lookup via REST API (Spine C)
  ...
  Scenario Outline: Look up current assignment for a known mention
    ...
    When I GET /lookup with source_id "<source_id>", request_id "<request_id>", entity_type "<entity_type>"
    ...
    Examples:
      ...
      | SYSTEM_A  | req-001    | ORGANISATION | cluster-010 | 2026-03-15T10:00:00Z | 200         |
      | SYSTEM_A  | req-002    | ORGANISATION | cluster-011 | 2026-03-15T11:30:00Z | 200         |
      | SYSTEM_B  | req-003    | ORGANISATION | cluster-012 | 2026-03-16T09:00:00Z | 200         |
  ...
  Scenario: Return not found when the mention triad is unknown
    ...
    When I GET /lookup with source_id "SYSTEM_UNKNOWN", request_id "req-999", entity_type "ORGANISATION"
  ...
  Scenario Outline: Reject single lookup with missing or empty query parameters
    Given a GET /lookup request with <param_condition>
  ...
  # source. The synchronisation snapshot is advanced only on success.
  # ---------------------------------------------------------------------------

  Scenario Outline: Retrieve changed assignments since the last synchronisation snapshot
    Given source "<source_id>" has <total_mentions> resolved mentions
    And <changed_count> mentions have been updated since the last synchronisation snapshot
    When I POST to /refreshBulk for source "<source_id>" with limit <limit>
    Then the response HTTP status is <http_status>
    And the response body contains <expected_count> delta assignments
    And each delta includes canonical_entity_id, source_id, request_id, entity_type, and update timestamp
    And the response body has_more is <has_more>
    And the synchronisation snapshot for "<source_id>" is advanced

    Examples:
      | source_id | total_mentions | changed_count | limit | expected_count | has_more | http_status |
      | SYSTEM_C  | 10             | 3             | 1000  | 3              | false    | 200         |
      | SYSTEM_C  | 10             | 0             | 1000  | 0              | false    | 200         |
      | SYSTEM_D  | 500            | 200           | 50    | 50             | true     | 200         |

  Scenario: First bulk lookup for a source returns all assignments and creates a synchronisation snapshot
    Given source "SYSTEM_NEW" has 4 resolved mentions in the Decision Store
    And source "SYSTEM_NEW" has no prior synchronisation snapshot
    When I POST to /refreshBulk for source "SYSTEM_NEW" with no continuation cursor
    Then the response HTTP status is 200
    And the response body contains 4 delta assignments
    And a synchronisation snapshot is created for source "SYSTEM_NEW"

  Scenario: Page through a large delta set until exhausted
    Given source "SYSTEM_E" has 7 mentions updated since the last synchronisation snapshot
    When I POST to /refreshBulk for source "SYSTEM_E" with limit 3
    Then the response HTTP status is 200
    ...
    When I POST to /refreshBulk for source "SYSTEM_E" using the returned continuation cursor with limit 3
    Then the response HTTP status is 200
    And the response body contains 3 delta assignments
    And the response body has_more is true
    When I POST to /refreshBulk for source "SYSTEM_E" using the returned continuation cursor with limit 3
  ...
  Scenario: Default page size is applied when limit is omitted
    Given source "SYSTEM_F" has 500 mentions updated since the last synchronisation snapshot
    When I POST to /refreshBulk for source "SYSTEM_F" without specifying a limit
    Then the response HTTP status is 200
  ...
  Scenario Outline: Reject bulk lookup with invalid request fields
    Given a refreshBulk request with <field_condition>
  ...
  Scenario: Return service error when Decision Store is unavailable for single lookup
    Given a mention with triad "SYSTEM_G", "req-060", "ORGANISATION" exists in the Decision Store
    And the Decision Store is unavailable
    When I GET /lookup with source_id "SYSTEM_G", request_id "req-060", entity_type "ORGANISATION"
    Then the response HTTP status is 500
    And the response body error code is "SERVICE_ERROR"

  Scenario: Return service error on bulk lookup and do not advance the synchronisation snapshot
    Given source "SYSTEM_H" has resolved mentions in the Decision Store
    And the Decision Store raises an error during the delta query
    When I POST to /refreshBulk for source "SYSTEM_H"
    Then the response HTTP status is 500
    And the response body error code is "SERVICE_ERROR"
    And the synchronisation snapshot for "SYSTEM_H" is not modified
  ...
  Scenario: Lookup operations do not modify assignments or trigger resolution
    ...
    And source "SYSTEM_I" has 2 mentions updated since the last synchronisation snapshot
    When I GET /lookup with source_id "SYSTEM_I", request_id "req-070", entity_type "ORGANISATION"
    And I POST to /refreshBulk for source "SYSTEM_I" with limit 1000
