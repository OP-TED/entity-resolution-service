...
def _build_service_with_decisions(
    ...
    async def _find_mentions(identifiers):
        return [identifier_map[d.id] for d in decisions if d.about_entity_mention in identifiers]
...
async def test_bulk_placement_recommendation(n: int) -> None:
    ...
    decisions = [
        _make_decision(f"dec-{i}", _make_identifier(f"src-{i}", f"req-{i}")) for i in range(n)
    ]
